...
//不使用
public class QueueStream<E> implements BlockingQueue<E> {
    public static Map<String,QueueStream> map=new HashMap<>();

    public static int defsize=120;
    ...
    public QueueStream(String username,int cap) throws Exception {
        this.cap=cap;
    ...
    public QueueStream(String username,int cap, int id) throws Exception {
        this.cap=cap;
        this.list=new ArrayList(cap);
        bytes=new byte[cap][];
...
//        this.senders.sendSym(s.getBytes());
    ...
    public byte[] read(){
        ...
        return Arrays.copyOfRange(bytesr,5,bytesr.length);
...
//                        }
                        try {
                            ...
                            System.out.println("(QueueStream)  re"+re.length);
                            System.out.println("(QueueStream)  "+Arrays.toString(re));
                        }


                    }else if (re[0]==-128){
                        synchronized (queueStream){
                            bytes[re[1]+128]=null;
                            p1=re[1]+128;
                        }
                    }else if (re[0]==127){
